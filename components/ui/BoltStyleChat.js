...
// CHAT INPUT
function ChatInput({ onSend, placeholder = "What do you want to build?", disabled = false }) {
      ...
      <div className="relative rounded-2xl bg-[#1e1e22] ring-1 ring-white/[0.08] shadow-[0_0_0_1px_rgba(255,255,255,0.05),0_2px_20px_rgba(0,0,0,0.4)]">
        <div className="relative">
          <textarea
            ...
            disabled={disabled}
            className="w-full resize-none bg-transparent text-[15px] text-white placeholder-[#5a5a5f] px-5 pt-5 pb-3 focus:outline-none min-h-[80px] max-h-[200px] disabled:opacity-50 disabled:cursor-not-allowed"
            style={{ height: '80px' }}
          />
        </div>
        ...
        <div className="flex items-center justify-between px-3 pb-3 pt-1">
          <div className="flex items-center gap-1">
            <div className="relative">
              <button
                ...
                disabled={disabled}
                className="flex items-center justify-center size-8 rounded-full bg-white/[0.08] hover:bg-white/[0.12] text-[#8a8a8f] hover:text-white transition-all duration-200 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
              >
          ...
          <div className="flex items-center gap-2">
            <button 
              disabled={disabled}
              className="flex items-center gap-1.5 px-3 py-2 rounded-full text-xs font-medium text-[#6a6a6f] hover:text-white hover:bg-white/5 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
            >
...
// IMPORT BUTTONS COMPONENT (Optional - can be hidden for sermon/study generation)
function ImportButtons({ onImport, show = false }) {
  if (!show) return null
  ...
  return (
    <div className="relative flex flex-col items-center justify-center min-h-screen w-full overflow-hidden bg-[#0f0f0f]">
      ...
      <div className="absolute top-[66%] left-1/2 sm:top-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center justify-center w-full h-full overflow-hidden px-4">
        ...
        {/* Import buttons */}
        <ImportButtons onImport={onImport} show={showImportButtons} />
      </div>
    </div>
  )
}
